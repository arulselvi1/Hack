  ...
  getdata(url)
    .then(async (data) => {
      document.getElementById("resultheading").innerText =
        type == "myplaylist" ? "My Playlists" : "Featured Playlists";
      ...
      document.getElementById("next").onclick = () => {
        getplaylists(
          d.next + `&access_token=${access_token}`,
          access_token,
          user_id,
          type
        );
      ...
      for (let i = 0; i < d.items.length; i++) {
        ...
        col.onclick = () => {
          getsongs(
            d.items[i].tracks.href +
              `?access_token=${access_token}&offset=0&limit=9`,
            access_token,
            maincol,
            1,
            user_id,
        ...
        let checkfollowed = await checkfollow(
          d.items[i].id,
          access_token,
          user_id
        );
